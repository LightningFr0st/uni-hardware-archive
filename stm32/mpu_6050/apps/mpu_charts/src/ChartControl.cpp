...
void ChartControl::set_capacity(::std::size_t cap)
{
	using namespace ::std;

	lock_guard<mutex> lock(mtx_);
	capacity_ = max<size_t>(2, cap);
	for (auto& s : series_) {
...
void ChartControl::OnPaint(wxPaintEvent&)
{
	...
	dc.Clear();
	auto gc(unique_ptr<wxGraphicsContext>(wxGraphicsContext::Create(dc)));
	if (!gc) return;
	...
	size_t maxN = 0;
	for (auto& s : local) maxN = max(maxN, s.data.size());
	if (maxN < 2) return;
	...
	const double marginTop = max(H / 12.0, minMargin * 2.0 + text_h);
	...
	if (!manual) {
		...
		for (auto& s : local) for (double v : s.data) { mn = min(mn, v); mx = max(mx, v); }
