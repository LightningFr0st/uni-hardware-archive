...
signal message : uint512_t := (others => '0');
...
function left_rotation(x: in uint32_t; c: in uint8_t) return uint32_t is
begin
	return SHIFT_LEFT(x, to_integer(c)) or SHIFT_RIGHT(x, to_integer(32-c));
end function left_rotation;

function invert_endianness(x: in uint32_t) return uint32_t is
begin
    return x(7 downto 0) & 
           x(15 downto 8) &
           x(23 downto 16) &
           x(31 downto 24);
end function invert_endianness;
    ...
    begin
        if (reset = '0' and rising_edge(clk)) then
            ...
            case state is
                ...
                when load_data =>
                    message(data_counter to data_counter+31) <= unsigned(data_in);
                    if (data_counter < message_length) then
                        data_counter <= data_counter + 32;
                    end if;

                when pad =>
                    message(to_integer(message_length)) <= '1';
                    message(to_integer(message_length+1) to 447) <= (others => '0');
                    message(448 to 511) <= 
                    invert_endianness(message_length) & "00000000000000000000000000000000";

                when rotate => 
                    for i in 0 to 15 loop
                        message(32*i to 32*i+31) <= invert_endianness(message(32*i to 32*i+31));
                    end loop;

                when stage1_B | stage2_B | stage3_B | stage4_B =>
                    A_n <= D;
                    B_n <= B + left_rotation(A + F + K(loop_counter) + message(g to g+31), s(loop_counter)); 
                    C_n <= B;
                    D_n <= C;
                ...
                when stage6 =>
                    A_n <= invert_endianness(A_n);
                    B_n <= invert_endianness(B_n);
                    C_n <= invert_endianness(C_n);
                    D_n <= invert_endianness(D_n);
